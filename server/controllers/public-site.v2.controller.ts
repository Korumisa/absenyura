...
export const upsertAdminProfile = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const payload = {
      ...
      home_card_left_title: String((data as any).homeCardLeftTitle ?? '').trim() || null,
      home_card_left_body: String((data as any).homeCardLeftBody ?? '').trim() || null,
      home_card_right_title: String((data as any).homeCardRightTitle ?? '').trim() || null,
      home_card_right_body: String((data as any).homeCardRightBody ?? '').trim() || null,
      vision: String((data as any).vision ?? '').trim() || null,
      mission: String((data as any).mission ?? '').trim() || null,
      visi_photo_url: String((data as any).visiPhotoUrl ?? '').trim() || null,
      visi_name: String((data as any).visiName ?? '').trim() || null,
      visi_role: String((data as any).visiRole ?? '').trim() || null,
      misi_photo_url: String((data as any).misiPhotoUrl ?? '').trim() || null,
