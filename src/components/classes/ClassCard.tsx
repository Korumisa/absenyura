...
import { classDetailLine } from '@/lib/utils/classLabel';
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="relative flex items-start gap-3 pr-14">
          <div
            className="grid size-12 shrink-0 place-items-center rounded-full bg-brand/10 text-brand"
            aria-hidden
          >
            <GraduationCap className="size-6" />
          </div>
          <div className="min-h-[5.25rem] min-w-0 flex-1 pt-0.5">
            <h3 className="line-clamp-1 text-base font-bold leading-snug text-foreground">
              {classItem.name}
            </h3>
        ...
        <div className="mt-auto border-t border-border pt-4">
          ...
          <p className="mt-1 min-h-[1.125rem] text-xs text-muted-foreground">
            {classItem._count.sessions > 0 ? `${classItem._count.sessions} sesi absensi` : '\u00a0'}
          </p>
        </div>
      </div>
    </article>
