        ...
        <PublicReveal className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          {isLoadingStructure ? (
            ...
            (() => {
              ...
              const core = ordered.filter((g) => isCore(g.title) || (Number(g.sort_order ?? 999) || 999) === 1);
        ...
        <PublicReveal className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          {isLoadingLomba ? (
            ...
            (() => {
              ...
              const count = shown.length;
              const gridClass =
                count <= 1 ? 'mt-10 grid gap-4 max-w-3xl mx-auto' : 'mt-10 grid gap-4 lg:grid-cols-2';
              return (
                <div className={gridClass}>
                  {shown.map((l) => (
                    <Link
                      key={l.id}
                      to="/informasi-lomba"
                      className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-black/10 bg-white shadow-[0_18px_45px_-42px_rgba(15,23,42,0.35)] transition hover:-translate-y-0.5 hover:border-[var(--public-primary)]/30"
                    >
                      ...
                      <div className="flex flex-1 items-start gap-5 p-5">
                        <div className="relative h-[112px] w-[84px] shrink-0 overflow-hidden rounded-xl border border-black/10 bg-slate-50">
                          <PublicCoverImage url={l.cover_image_url} alt={l.title} imgClassName="transition duration-500 group-hover:scale-[1.02]" />
                          <div className="pointer-events-none absolute inset-0 ring-1 ring-inset ring-black/5" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <div className="text-sm font-extrabold tracking-tight text-slate-900 line-clamp-2">{l.title}</div>
                          <div className="mt-2 text-xs font-semibold uppercase tracking-widest text-slate-500">
                            {l.date_label ? `Batas: ${l.date_label}` : 'Batas: -'}
                          </div>
                          {l.excerpt ? (
                            <div className="mt-3 text-sm text-slate-700 line-clamp-2">{l.excerpt}</div>
                          ) : (
                            <div className="mt-3 text-sm text-slate-500 line-clamp-2">Ringkasan belum tersedia.</div>
                          )}
                          <div className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[var(--public-primary)] px-4 py-2 text-xs font-semibold text-white shadow-[0_12px_22px_rgba(37,99,235,0.28)] transition group-hover:brightness-110">
                            Lihat Detail
                            <ArrowRight size={16} />
                          </div>
                        </div>
